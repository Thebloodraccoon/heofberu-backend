...
class CharacterFeatService:
    ...
    Every write here (add/update/remove) recalculates and upserts the
    ability-score cache before returning, since granting or changing a
    feat's ASI choice can change the character's effective ability
    scores — see ``CharacterAbilityScoreCalculator``.
    """
    ...
    def add_feat(self, character_id: int, data: CharacterFeatAdd, current_user: UserResponse) -> CharacterFeatResponse:
        ...
        existing = self.repository.get_character_feat_by_feat_id(character_id, data.feat_id)
        if existing:
        ...
        grant = self.repository.add_character_feat(character_id, data.feat_id, data.ability_score_increase_id)
    ...
    def remove_feat(self, character_id: int, character_feat_id: int, current_user: UserResponse) -> bool:
        ...
        grant = self._get_grant_or_404(character_id, character_feat_id)
        result = self.repository.remove_character_feat(grant)
    ...
    def _get_grant_or_404(self, character_id: int, character_feat_id: int) -> CharacterFeat:
        grant = self.repository.get_character_feat_by_id(character_id, character_feat_id)
        if not grant:
            raise CharacterFeatNotFoundException(character_id=character_id, character_feat_id=character_feat_id)
        return grant

    def _validate_ability_score_increase(self, feat: Feat, ability_score_increase_id: int) -> None:
        """
    ...
    def _check_prerequisite(self, character, feat: Feat) -> None:
        """
    ...
    def _recalculate_ability_cache(self, character) -> None:
        """Recompute and persist the effective-ability-score cache for ``character``."""

        totals = self.ability_calculator.compute(character)
        self.repository.upsert_ability_score_cache(character.id, totals)
