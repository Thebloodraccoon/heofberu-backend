...
def check_feat_prerequisite(
    character: Character, feat: Feat, ability_cache_service: CharacterAbilityCacheService
) -> None:
    """
    ...
    stale. See ``CharacterAbilityCacheService.compute``.
    """

    if feat.prerequisite_ability is None or feat.prerequisite_minimum_score is None:
        return

    totals = ability_cache_service.compute(character)
