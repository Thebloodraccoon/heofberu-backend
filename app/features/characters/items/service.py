"""Character item service: managing a character's inventory."""

from sqlalchemy.orm import Session

from app.features.characters.base import CharacterSubDomainService
from app.features.characters.items.exceptions import CharacterItemNotFoundException
from app.features.characters.items.repository import CharacterItemRepository
from app.features.characters.items.schemas import (
    CharacterItemAdd,
    CharacterItemResponse,
    CharacterItemUpdate,
)
from app.features.items.exceptions import ItemNotFoundException
from app.features.items.repository import ItemRepository
from app.features.users.schemas import UserResponse
from app.models.character_item_model import CharacterItem


class CharacterItemService(CharacterSubDomainService):
    """
    Manage the items a character owns (``character_items``).

    Each row is an independent stack of an item, so the same item may be
    owned several times. Uses three collaborators:

      - the inherited ``CharacterSubDomainService`` — access control
        only (fetching the owning character to check GM/owner permission
        via ``get_character_for_user``); no inventory data lives there.
      - ``CharacterItemRepository`` — the ``character_items`` stack rows
        (CRUD).
      - ``ItemRepository`` — looking up the reference item being added,
        so stacks always point at an existing item.
    """

    def __init__(self, db: Session):
        super().__init__(db)
        self.character_item_repository = CharacterItemRepository(db)
        self.item_repository = ItemRepository(db)

    def get_items(self, character_id: int, current_user: UserResponse) -> list[CharacterItemResponse]:
        """List every item stack owned by a character."""

        self.get_character_for_user(character_id, current_user)

        stacks = self.character_item_repository.get_character_items(character_id)
        return [CharacterItemResponse.model_validate(stack) for stack in stacks]

    def add_item(self, character_id: int, data: CharacterItemAdd, current_user: UserResponse) -> CharacterItemResponse:
        """Add an item stack to a character's inventory."""

        self.get_character_for_user(character_id, current_user)

        if not self.item_repository.exists_by_id(data.item_id):
            raise ItemNotFoundException(item_id=data.item_id)

        stack = self.character_item_repository.add_character_item(
            character_id,
            data.item_id,
            data.quantity,
            data.is_equipped,
            data.is_attuned,
            data.notes,
        )
        return CharacterItemResponse.model_validate(stack)

    def update_item(
        self,
        character_id: int,
        character_item_id: int,
        data: CharacterItemUpdate,
        current_user: UserResponse,
    ) -> CharacterItemResponse:
        """Change an item stack's quantity, state flags, or notes."""

        self.get_character_for_user(character_id, current_user)

        stack = self._get_stack_or_404(character_id, character_item_id)
        update_data = data.model_dump(exclude_unset=True)
        updated_stack = self.character_item_repository.update(stack, update_data, refresh=True)
        return CharacterItemResponse.model_validate(updated_stack)

    def remove_item(self, character_id: int, character_item_id: int, current_user: UserResponse) -> bool:
        """Remove an item stack from a character's inventory."""

        self.get_character_for_user(character_id, current_user)

        stack = self._get_stack_or_404(character_id, character_item_id)
        return self.character_item_repository.remove_character_item(stack)

    def _get_stack_or_404(self, character_id: int, character_item_id: int) -> CharacterItem:
        """Fetch an item stack scoped to the character, or raise ``CharacterItemNotFoundException``."""

        stack = self.character_item_repository.get_character_item_by_id(character_id, character_item_id)
        if not stack:
            raise CharacterItemNotFoundException(character_id=character_id, character_item_id=character_item_id)
        return stack
