...
class TestCharacterItems:
    async def test_add_and_list_item(self, client, player, player_token, create_class, create_character, create_item):
    ...
    async def test_add_missing_item_returns_404(self, client, player, player_token, create_class, create_character):
        character_class = await create_class(name="Fighter")
        character = await create_character(owner_id=player.id, class_id=character_class.id)
    ...
    async def test_update_item_stack(self, client, player, player_token, create_class, create_character, create_item):
        character_class = await create_class(name="Fighter")
        character = await create_character(owner_id=player.id, class_id=character_class.id)
        item = await create_item(name="Longsword")
        add_response = await client.post(
    ...
    async def test_remove_item(self, client, player, player_token, create_class, create_character, create_item):
        character_class = await create_class(name="Fighter")
        character = await create_character(owner_id=player.id, class_id=character_class.id)
        item = await create_item(name="Longsword")
        add_response = await client.post(
    ...
    async def test_player_cannot_add_item_to_other_players_character(
